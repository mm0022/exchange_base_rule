...
4. **后续运行**：与基线对比 —— 以"更新于"(updateTime) 为权威变更信号，日期前移的文档列出并给逐字 diff（diff 取自重新抓取的正文）；新增/删除的文档也列出。
   - 说明：updateTime 是 OKX 文档被编辑时刷新的时间戳，正文变更必然伴随 updateTime 前移；故不额外把"正文 diff"作为独立触发条件（避免每次重抓全部正文）。

### 4.2 费率规则
- 抓正文存快照。首次为基线；后续与基线对比给 diff。无日期信号，仅靠内容 diff。

### 4.3 上下币公告
- 通过官方 API 按 annType（上币/下币）翻页抓取，直到 `pTime` 早于窗口起点为止。
- 每次运行（含首次）都列出近 N 天（默认 3，即 2026-06-27 至今）发布的公告——这是滚动窗口，与基线无关（对应用户需求"从前三天到此刻的"）。
- 按上币 / 下币分类汇总：标题、日期、链接。
- 注：不做"距上次运行新增"的标记（YAGNI；用户只要求滚动窗口）。如需该功能可后续添加。
...
- 快照：单文件 `snapshots/okx.json`，两个顶层键 `docs`（slug→{title, update_time, body}）与 `fees_text`（稳定的 feeDataInfo 序列化）。
